/**
 * FLAG(M10): PRINTER_APP_URL must point at the frontend origin that serves `/printer/$resumeId`.
 * Browserless loads this URL; it is not the backend API origin.
 */
export function buildPrinterPageUrl(resumeId: string, token: string): string {
  const baseUrl = env.PRINTER_APP_URL ?? env.APP_URL;
  const url = new URL(`/printer/${resumeId}`, baseUrl);
...
async function doPrintResumeAsPDF(
  ...
  try {
    ...
    page = await context
      ...
      .catch((error) =>
        throwPrinterStepError(
          "Failed to open a new browser page for PDF generation",
          "Retry the request",
          error,
        ),
      );
    ...
    await page
      ...
      .catch((error) =>
        throwPrinterStepError(
          "Failed to switch browser page to print media mode",
          "Retry the request",
          error,
        ),
      );
    await page
      ...
      .catch((error) =>
        throwPrinterStepError(
          "Failed to apply PDF viewport dimensions",
          "Retry the request",
          error,
        ),
      );
    ...
    const pdfHeight =
      isFreeForm && contentHeight ? contentHeight : pageDimensionsAsPixels[format].height;
...
async function doGetResumeScreenshot(
  ...
  if (existingScreenshots.length > 0) {
    ...
    if (sortedFiles.length > 0) {
      ...
      await Promise.all(sortedFiles.map((file) => storageService.delete(file.path))).catch(
        (error) =>
          throwPrinterStepError(
  ...
  try {
    ...
    page = await context
      ...
      .catch((error) =>
        throwPrinterStepError(
          "Failed to open a new browser page for screenshot capture",
          "Retry the request",
          error,
        ),
      );
    ...
    await page
      ...
      .catch((error) =>
        throwPrinterStepError(
          "Failed to apply screenshot viewport dimensions",
          "Retry the request",
          error,
        ),
      );
