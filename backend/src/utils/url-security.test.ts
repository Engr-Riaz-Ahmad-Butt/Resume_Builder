...
describe("isAllowedExternalUrl", () => {
  ...
  it("supports origin-only entries in the allow list", () => {
    ...
    expect(isAllowedExternalUrl("https://sub.example.org/v1/hello", originOnlyAllowedHosts)).toBe(
      false,
    );
  });
});
...
describe("isAllowedOAuthRedirectUri", () => {
  ...
  it("allows local loopback HTTP callbacks for native OAuth clients", () => {
    expect(
      isAllowedOAuthRedirectUri("http://localhost:6188/callback", trustedOrigins, allowedHosts),
    ).toBe(true);
    expect(
      isAllowedOAuthRedirectUri("http://127.0.0.1:6188/callback", trustedOrigins, allowedHosts),
    ).toBe(true);
    expect(
      isAllowedOAuthRedirectUri("http://[::1]:6188/callback", trustedOrigins, allowedHosts),
    ).toBe(true);
  });

  it("keeps rejecting unsafe HTTP and malformed redirects", () => {
    expect(
      isAllowedOAuthRedirectUri("http://192.168.1.20:6188/callback", trustedOrigins, allowedHosts),
    ).toBe(false);
    expect(
      isAllowedOAuthRedirectUri("http://example.com/callback", trustedOrigins, allowedHosts),
    ).toBe(false);
  ...
  it("allows only trusted HTTPS redirects without credentials or fragments", () => {
    expect(
      isAllowedOAuthRedirectUri("https://rxresu.me/callback", trustedOrigins, allowedHosts),
    ).toBe(true);
    expect(
      isAllowedOAuthRedirectUri(
    ...
    expect(
      isAllowedOAuthRedirectUri("https://evil.example.com/callback", trustedOrigins, allowedHosts),
    ).toBe(false);
    expect(
      isAllowedOAuthRedirectUri(
        "https://user:pass@rxresu.me/callback",
        trustedOrigins,
        allowedHosts,
      ),
    ).toBe(false);
    expect(
      isAllowedOAuthRedirectUri("https://rxresu.me/callback#token", trustedOrigins, allowedHosts),
    ).toBe(false);
  });
});
...
describe("sanitizeResumePictureUrl", () => {
  ...
  it("keeps local uploads paths", () => {
    expect(sanitizeResumePictureUrl("/uploads/user/pictures/a.webp", appUrl)).toBe(
      "/uploads/user/pictures/a.webp",
    );
  });

  it("converts same-origin upload URLs to path-only values", () => {
    expect(sanitizeResumePictureUrl("https://rxresu.me/uploads/u/p.jpg", appUrl)).toBe(
      "/uploads/u/p.jpg",
    );
  });
...
describe("parse helpers", () => {
  ...
  it("parseAllowedHostList normalizes host entries and handles empty input", () => {
    const set = parseAllowedHostList(
      " API.OpenAI.com , https://gateway.ai.vercel.com ,,api.openai.com ",
    );
