...
function sanitizePlainTextContent(value: string): string {
  return DOMPurify.sanitize(value, PLAIN_TEXT_SANITIZE_CONFIG);
}
...
function decodeCssEscapes(value: string): string {
  ...
  return value.replace(
    ...
    (_match, hex, escapedChar) => {
      if (hex) return String.fromCodePoint(parseInt(hex as string, 16));
      return (escapedChar as string | undefined) ?? "";
    },
  );
}
...
export function sanitizeHtml(html: string): string {
  if (!html) return "";
  return DOMPurify.sanitize(html, { ...RICH_TEXT_CONFIG } as Parameters<
