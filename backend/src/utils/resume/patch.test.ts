...
describe("jsonPatchOperationSchema", () => {
  ...
  it("accepts 'add' with undefined value (z.any().optional() allows it)", () => {
    // FLAG(Zod4): z.any() alone rejects missing keys; .optional() preserves prior intent
...
describe("applyResumePatches", () => {
  describe("basic operations", () => {
    ...
    it("adds an item to a section's items array", () => {
      ...
      const result = applyResumePatches(data, [
        { op: "add", path: "/sections/skills/items/-", value: newSkill },
      ]);
  ...
  describe("test operation", () => {
    ...
    it("throws ResumePatchError when test value does not match", () => {
      ...
      expect(() =>
        applyResumePatches(data, [{ op: "test", path: "/basics/name", value: "Wrong" }]),
  ...
  describe("copy and move operations", () => {
    it("copies a value from one path to another", () => {
      ...
      const result = applyResumePatches(data, [
        { op: "copy", path: "/basics/headline", from: "/basics/name" },
      ]);
  ...
  describe("metadata operations", () => {
    it("replaces the template", () => {
      const data = freshResume();
      const result = applyResumePatches(data, [
        { op: "replace", path: "/metadata/template", value: "pikachu" },
      ]);
    ...
    it("toggles section visibility", () => {
      const data = freshResume();
      const result = applyResumePatches(data, [
        { op: "replace", path: "/sections/skills/hidden", value: true },
      ]);
    ...
    it("replaces page margins", () => {
      const data = freshResume();
      const result = applyResumePatches(data, [
        { op: "replace", path: "/metadata/page/marginX", value: 20 },
      ]);
  ...
  describe("custom sections", () => {
    it("adds a custom section", () => {
      ...
      const result = applyResumePatches(data, [
        { op: "add", path: "/customSections/-", value: customSection },
      ]);
  ...
  describe("picture operations", () => {
    ...
    it("updates picture size within valid range", () => {
      const data = freshResume();
      const result = applyResumePatches(data, [
        { op: "replace", path: "/picture/size", value: 120 },
      ]);
  ...
  describe("error handling", () => {
    it("throws ResumePatchError for invalid path", () => {
      ...
      expect(() =>
        applyResumePatches(data, [{ op: "replace", path: "/nonexistent/path", value: "x" }]),
      ).toThrow(ResumePatchError);
    });
    ...
    it("throws ResumePatchError for unresolvable 'from' in move", () => {
      ...
      expect(() =>
        applyResumePatches(data, [
          { op: "move", path: "/basics/name", from: "/nonexistent/field" },
        ]),
      ).toThrow(ResumePatchError);
    });
    ...
    it("throws ResumePatchError for out-of-bounds array index", () => {
      ...
      expect(() =>
        applyResumePatches(data, [{ op: "remove", path: "/sections/skills/items/999" }]),
      ).toThrow(ResumePatchError);
    });
    ...
    it("throws when patch produces invalid resume data (e.g. wrong type)", () => {
      ...
      expect(() =>
        applyResumePatches(data, [
          { op: "replace", path: "/picture/hidden", value: "not-a-boolean" },
        ]),
      ).toThrow();
    });
    ...
    it("throws when patch produces invalid resume data (picture size out of range)", () => {
      ...
      expect(() =>
        applyResumePatches(data, [{ op: "replace", path: "/picture/size", value: 9999 }]),
      ).toThrow();
    });
    ...
    it("returns a valid ResumeData after patching", () => {
      const data = freshResume();
      const result = applyResumePatches(data, [
        { op: "replace", path: "/basics/name", value: "Valid" },
      ]);
  ...
  describe("layout operations", () => {
    it("replaces sidebar width", () => {
      const data = freshResume();
      const result = applyResumePatches(data, [
        { op: "replace", path: "/metadata/layout/sidebarWidth", value: 40 },
      ]);
    ...
    it("adds a new page to layout", () => {
      ...
      const result = applyResumePatches(data, [
        { op: "add", path: "/metadata/layout/pages/-", value: newPage },
      ]);
  ...
  describe("schema validation after patch", () => {
    it("throws on negative picture size (below min)", () => {
      const data = freshResume();
      expect(() =>
        applyResumePatches(data, [{ op: "replace", path: "/picture/size", value: -1 }]),
      ).toThrow();
    });
    ...
    it("falls back to default for invalid page format (schema uses .catch)", () => {
      ...
      const result = applyResumePatches(data, [
        { op: "replace", path: "/metadata/page/format", value: "tabloid" },
      ]);
    ...
    it("allows setting customSections to empty array", () => {
      const data = freshResume();
      data.customSections = [
        { id: "c1", type: "experience", title: "X", columns: 1, hidden: false, items: [] },
      ];

      const result = applyResumePatches(data, [
        { op: "replace", path: "/customSections", value: [] },
      ]);
